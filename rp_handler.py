...
def save_base64_image(base64_string: str) -> dict:
    try:
        output_dir = os.environ.get('COMFY_INPUT_PATH', '/comfyui/input')
        header, encoded = base64_string.split(',', 1)
        image_format = header.split(':')[1].split(';')[0].split('/')[1]
...
def clear_temp_data():
    try:
        ...
        comfy_output_path = os.environ.get('COMFY_OUTPUT_PATH', '/comfyui/output')
        for filename in os.listdir(comfy_input_path):
...
def handler(job):
    ...
    save_result = save_base64_image(job_input["source_image"])
    if save_result["status"] == "error":
        clear_temp_data()
        return save_result
    else:
        workflow["3"]["inputs"]["image"] = save_result["file_path"]
    save_result = save_base64_image(job_input["garment_image"])
    if save_result["status"] == "error":
        clear_temp_data()
        return save_result
    else:
        workflow["4"]["inputs"]["image"] = save_result["file_path"]
    
    workflow["17"]["inputs"]["string"] = job_input["objects"]
